module topMbist(
    ...
    input wire march_en,   // Kích hoạt March C-
    ...
    // --- 1. Khai báo các tín hiệu điều khiển nội bộ ---
    ...
    // Tín hiệu địa chỉ từ các nguồn khác nhau
    ...
    // --- 6. Hệ thống MUX lựa chọn (Ưu tiên: March C- > Checkerboard > MSCAN) ---
    ...
    assign compSel_ctrl     = march_en ? cs_mc : (cb_en ? cs_c : (mscan_en ? cs_m : 4'b0000));
    assign rstComp_ctrl     = march_en ? rc_mc : (cb_en ? rc_c : (mscan_en ? rc_m : 1'b0)); 

    // MUX ĐỊA CHỈ: Lấy địa chỉ từ thuật toán đang hoạt động
    assign W_addr = march_en ? addr_from_march : (cb_en ? addr_from_cb : addr_from_gen); 

    // --- 7. Các khối chức năng chung ---
    addr_gen u_addr_gen (
    ...
    // --- 8. Hệ thống 4 bộ so sánh song song ---
